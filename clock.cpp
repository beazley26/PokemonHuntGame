...
void Clock::display() const { 
  ...
  static float oldTicks = 0;
  static float newTicks = 0;

  if ( getSeconds() > seconds ) {
    seconds = getSeconds();
  }
  if ( frames - oldFrames == (unsigned int)(Gamedata::getInstance().getXmlInt("fpsNum")) ) {
    newTicks=(float)getTicks();
    lastFrames=(frames-oldFrames)/((newTicks-oldTicks)/1000);
    oldFrames=frames;
    oldTicks=newTicks;
  }
  IOManager::getInstance()
    .printMessageValueAt("Seconds: ", seconds, 10, 10);
  IOManager::getInstance()
    .printMessageValueAt("Frames per Sec: ", lastFrames, 10, 25);
}
